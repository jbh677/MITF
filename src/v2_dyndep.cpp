...
Type objective_function<Type>::operator() ()
{
  ...
  for(int s=0;s<ns;s++) {
    for(int r=0;r<nr;r++) {
      ...
      N(0,0,s,r) = R0 * eta(r) * zeta(s); // recruitment
      N0(0,0,s,r) = N(0,0,s,r);
  ...
  for(int s=0;s<ns;s++) {
    for(int r=0;r<nr;r++) {
      ...
      SSB0(0,s,r) = Type(0);
      for(int a=0;a<na;a++) SSB0(0,s,r) += N0(0,a,s,r) * m(a,s) * w(a,s); 
  ...
  for(int y=1;y<ny;y++) {
    ...
    for(int s=0;s<ns;s++) {
      ...
      for(int a=1;a<na-1;a++) {
        for(int r=0;r<nr;r++) {
          ...
          for(int rr=0;rr<nr;rr++) N0(y,a,s,r) += pim(rr,r)*N0(y-1,a-1,s,rr)*exp(-M); 
      ...
      for(int r=0;r<nr;r++) {
          ...
          for(int rr=0;rr<nr;rr++) N0(y,na-1,s,r) += pim(rr,r)*N0(y-1,na-1,s,rr)*exp(-M)+pim(rr,r)*N0(y-1,na-2,s,rr)*exp(-M); 
      ...
      for(int r=0;r<nr;r++) {
        ...
        SSB0(y,s,r) = Type(0);
        for(int a=0;a<na;a++) SSB0(y,s,r) += N0(y,a,s,r) * m(a,s) * w(a,s); 
    ...
    SSB0ftot(y) = Type(0);
    for(int r=0;r<nr;r++) SSB0ftot(y) += SSB0(y,0,r); 
    dSSB(y) = SSBftot(y)/SSB0ftot(y); 
    Rtot(y) = Type(0);
    for(int r=0;r<nr;r++)
