...
psi <- 0.861 # immediate tag shedding probability
nu <- 0.0173 # continuous rate of tag shedding
...
obj <- MakeADFun(data=data,parameters=pars1,map=map1,DLL="v2")
res1 <- do.call("optim",obj)
rep1 <- obj$rep()
sdrep <- sdreport(obj)
summary(sdrep)
...
obj <- MakeADFun(data=data,parameters=pars2,map=map2,DLL="v2")
...
obj <- MakeADFun(data=data,parameters=pars3,map=map3,DLL="v2")
res3 <- do.call("optim",obj)
rep3 <- obj$rep()
sdrep <- sdreport(obj)
summary(sdrep)
...
obj <- MakeADFun(data=data,parameters=pars4,map=map4,DLL="v2")
res4 <- do.call("optim",obj)
rep4 <- obj$rep()
sdrep <- sdreport(obj)
summary(sdrep)
...
obj <- MakeADFun(data=data,parameters=pars5,map=map5,DLL="v2")
res5 <- do.call("optim",obj)
rep5 <- obj$rep()
sdrep <- sdreport(obj)
summary(sdrep)
...
obj <- MakeADFun(data=data,parameters=pars6,map=map6,DLL="v2")
res6 <- do.call("optim",obj)
rep6 <- obj$rep()
sdrep <- sdreport(obj)
summary(sdrep)

S <- sdrep$value[grep("SSBf",names(sdrep$value))]
dep <- S/S[1]
...
obj7 <- MakeADFun(data=data,parameters=pars7,map=map6,DLL="v2",random="epsR")
res7 <- do.call("optim",obj7)
rep7 <- obj7$rep()
sdrep7 <- sdreport(obj7)
summary(sdrep6)

# alternative using marginal profile likelihood method

vsig <- seq(0.4,0.6,length=10)

mnlogl <- est.sigmaR(data,pars6,"v2",vsig,sub=TRUE)
mnlogl <- mnlogl-min(mnlogl)
plot(vsig,mnlogl,type='b')

pars7 <- list(logR0 = res6$par[['logR0']],
             epsR = unname(res6$par[grep('epsR',names(res6$par))]),
             lsigmaR = log(0.302),
             ilogeta = res6$par[['ilogeta']],
             ilogpim =unname(res6$par[grep('ilogpim',names(res6$par))]),
             selparsATT = unname(res6$par[grep('selparsATT',names(res6$par))]),
...
obj <- MakeADFun(data=data,parameters=pars7,map=map6,DLL="v2")
res7 <- do.call("optim",obj)
rep7 <- obj$rep()
sdrep <- sdreport(obj)
summary(sdrep)
...
plot.agelendat(ALF,ALFcov,repp,'ATT','female')
...
# for now fix ODs at MLEs 

map8 <- list(lsigmaR = factor(NA), 
             xphiLF = rep(factor(NA),nf),
             xphiT = factor(NA) )

pars8 <- list(logR0 = res7$par[['logR0']],
              epsR = unname(res7$par[grep('epsR',names(res7$par))]),
              lsigmaR = log(0.302),
              ilogeta = res7$par[['ilogeta']],
              ilogpim =unname(res7$par[grep('ilogpim',names(res7$par))]),
              selparsATT = unname(res7$par[grep('selparsATT',names(res7$par))]),
              selparsNVT = unname(res7$par[grep('selparsNVT',names(res7$par))]), 
              selparsATL = unname(res7$par[grep('selparsATL',names(res7$par))]), 
              selparsNMRL = unname(res7$par[grep('selparsNMRL',names(res7$par))]), 
              selparsSMRL = unname(res7$par[grep('selparsSMRL',names(res7$par))]), 
              xphiLF = unname(res7$par[grep('xphiLF',names(res7$par))]),
              xphiT = res7$par[['xphiT']] 
)

objmc <- MakeADFun(data=data,parameters=pars8,map=map8,DLL="v2")
resmc <- do.call("optim",objmc)
...
nits <- 1000
burn <- 100
system.time(mcmc <- tmbstan(objmc, init = 'last.par.best', iter = nits+burn, warmup = burn, chains=1,control=list(max_treedepth=12)))

traceplot(mcmc, pars=names(objmc$par), inc_warmup=TRUE)
traceplot(mcmc, pars=names(objmc$par), inc_warmup=FALSE)
pairs(mcmc, pars=names(objmc$par)[c(1,33,34,35)])
parlist <- extract(mcmc)
npar <- length(resmc$par)
parmat <- matrix(nrow=nits,ncol=npar)
colnames(parmat) <- names(resmc$par)
