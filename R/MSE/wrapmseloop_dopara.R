...
tdflist <- foreach(i=1:nits) %dopar% wrap.get.tag.data2(i)
...
replist <- foreach(i=1:nits) %dopar% wrap.get.hrates(i)
...
for(j in 2:ndec) { 
  ...
  txdflist <- foreach(i=1:nits) %dopar% wrap.extend.tag.data(i)
  ...
  tnewdf <- foreach(i=1:nits) %dopar% wrap.get.tag.data2(i)
