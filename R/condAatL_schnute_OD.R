...
compile("condAatL_schnute_OD.cpp")
dyn.load(dynlib("condAatL_schnute_OD"))
...
phiLFmin <- 1.05
phiLFmax <- min(neff)-0.05
xphiLF <- log((2-phiLFmin)/(phiLFmax-2))
...
data <- list(lf=lf,neff=neff,alk=alk,lref=lref,aref=ages,alref=c(a1,a2),dms=dim(alk),A=A,priors=c(mupr,sdpr,musigma,sdsigma),phiLFmin=phiLFmin,phiLFmax=phiLFmax)
...
obj <- MakeADFun(data=data,parameters=pars,DLL="condAatL_schnute_OD")
